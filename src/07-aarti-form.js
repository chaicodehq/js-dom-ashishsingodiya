...
export function validateName(name) {
  ...
  if (!/^[A-Za-z ]+$/.test(name)) return { valid: false, error: "Naam mein sirf letters aur spaces allowed hain" };
...
export function setupAartiForm(formElement, onSuccess, onError) {
  ...
  const handleSubmit = (e) => {
    ...
    const aartiType = formData.get("aartiType");
    ...
    const aartiValidation = validateAartiType(aartiType);
    if (!aartiValidation.valid) errors.push(aartiValidation.error);
