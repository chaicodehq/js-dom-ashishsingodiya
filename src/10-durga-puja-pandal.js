...
export function createPandalElement(pandal) {
  ...
  if (name == null || zone == null || theme == null || budget == null || rating == null) return null;
...
export function filterPandalsByZone(container, zone) {
  ...
  const pandals = container.getElementsByClassName("pandal");
...
export function getPandalsByBudgetRange(container, min, max) {
  ...
  return Array.from(pandals).filter((p) => {
    return Number(p.dataset.budget) >= min && Number(p.dataset.budget) <= max;
  });
}
